...
plot(stnn, conf_int = TRUE, method = "deltamethod", x_axis_r = c(0, 1), 
     which = 4, cex.caption = 3, cex.lab = 3, cex.axis = 2, caption = "",
     xaxt="n", lwd = 2)
 axis(1, at = seq(0, 1, length.out = 5), labels = seq(3.5, 8.5, length.out = 5),
     cex.axis = 2)
 text(0.2, 0.15, paste0("p-value = ",
                        unlist(strsplit(s$coefdf$`Pr(> X^2)`[4],
                                        split=' ',
                                        fixed=TRUE))[1]), cex = 2)
 ...
 par(mar=c(5,6,1,1)+.1)
 plot(stnn, conf_int = TRUE, method = "deltamethod", x_axis_r = c(0, 1), 
      which = 8, cex.caption = 3, cex.lab = 2.5, cex.axis = 2, caption = "",
      xaxt="n", lwd = 2)
 axis(1, at = seq(0, 1, length.out = 5), labels =  seq(2, 38, length.out = 5),
      cex.axis = 2)
 text(0.8, -0.1, paste0("p-value = ",
                        unlist(strsplit(s$coefdf$`Pr(> X^2)`[8],
                                        split=' ',
                                        fixed=TRUE))[1]), cex = 2)

 
 # change to plot.statnn needed to allow lwd for abline(h = 0)
